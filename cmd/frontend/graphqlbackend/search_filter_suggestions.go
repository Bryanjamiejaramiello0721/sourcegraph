...
func (r *schemaResolver) SearchFilterSuggestions() (*searchFilterSuggestions, error) {
	return &searchFilterSuggestions{}, nil
}
...
func (s *searchFilterSuggestions) Filters() []descriptiveValue {
	return []descriptiveValue{
		...
		{
			value:       "repogroup",
			description: "group-name (only include results from the named group)",
		},
		{
			value:       "repohasfile",
		...
		{
			value:       "lang",
			description: "lang-name (only include results from the named language)",
		},
		{
			value:       "fork",
...
func (s *searchFilterSuggestions) File() []string {
	return []string{
		`(test|spec)`,
		`\.json$`,
		`(vendor|node_modules)/`,
...
func (s *searchFilterSuggestions) Repohasfile() []string { return s.repohasfile }
